...
| D9 | B8 consensus-params source | **Compile-anchored (founder-approved 2026-07-07)** — params come from the compiled `default_genesis()`, threaded via `set_consensus_params` + `refuse_to_bind` hard gate; `genesis.json`'s `consensus_params` section is inert on the run-path (no `--genesis` file-load machinery in v1). All nodes on a binary agree (more deterministic for a coordinated-upgrade flip); operator param-tuning via genesis.json is a deferred refinement. Supersedes C6's literal file-load requirement. |
